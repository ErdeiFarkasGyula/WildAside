...
public class ModBoatRenderer extends BoatRenderer {
    ...
    public ModBoatRenderer(EntityRendererProvider.Context pContext, boolean pChestBoat) {
        ...
        this.boatResources = Stream.of(ModBoatEntity.Type.values()).collect(ImmutableMap.toImmutableMap(type -> type,
                type -> Pair.of(ResourceLocation.fromNamespaceAndPath(WildAside.MOD_ID, getTextureLocation(type, pChestBoat)), this.createBoatModel(pContext, type, pChestBoat))));
    }
    ...
    private static ModelLayerLocation createLocation(String pPath, String pModel) {
        return new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(WildAside.MOD_ID, pPath), pModel);
    }
